...
class CMULocalizationInterface {
 ...
 private:
  ...
  // VISLocalizationInterface: world_camera is the frame_id of the localization odometry (VLOC)
  Eigen::Matrix4d processOdometry(const nav_msgs::Odometry::ConstPtr &msg) {
    ...
    Eigen::Matrix4d T_world_base_base =
        T_base_sensor * T_world_pc_pc * T_base_sensor.inverse();
  ...
  void processPointCloud(const sensor_msgs::PointCloud2::ConstPtr &cloud_msg,
    ...
    for (size_t i = 0; i < cloud->size(); i += point_skip) {
      ...
      Eigen::Vector3d point_base = T_base_sensor.block<3, 3>(0, 0) * point + T_base_sensor.block<3, 1>(0, 3);

      if ((abs(point_base.x()) < near_point_filter_length) && (abs(point_base.y()) < near_point_filter_width) &&
          (abs(point_base.z()) < 0.5))
        continue;

      // conver the point in the base frame to the world base frame
      Eigen::Vector3d point_world_base =
          T_world_base_base.block<3, 3>(0, 0) * point_base + T_world_base_base.block<3, 1>(0, 3);
      PointType p_world = p;
...
class VISLocalizationInterface : public CMULocalizationInterface {
 public:
  VISLocalizationInterface(ros::NodeHandle &nh, ros::NodeHandle &nhp)
      : CMULocalizationInterface(nh, nhp) {
    // clang-format off
    ...
    sync.registerCallback(boost::bind(&VISLocalizationInterface::callback, this, _1, _2));
    // clang-format on
  }
